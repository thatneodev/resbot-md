...
const { logWithTime, getCurrentDate, sendMessageWithMentionNotQuoted,sendImagesWithMentionNotQuoted, getCurrentTime, getGreeting, getHari, logTracking } = require('@lib/utils');
  ...
  try {
      ...
      if (status === 'blacklist') {
          ...
          if (fitur.detectblacklist2) {
            logTracking(`Participant Update - Peringatan Blacklist2 di kick (${sender})`);
