...
async function checkAndInstallModule(moduleName) {
    try {
        ...
        await new Promise((resolve, reject) => {
            const install = spawn('npm', ['install', moduleName], { stdio: 'inherit', shell: true });
...
async function checkAndInstallModules(modules) {
    for (const module of modules) {
        await checkAndInstallModule(module);
    }
}
