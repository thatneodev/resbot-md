...
async function handle(sock, messageInfo) {
    const { remoteJid, sender, isGroup, message, content } = messageInfo;
    if (!isGroup) return;
    ...
    if (!content) {
        ...
        if (game.players.length === 0) {
            ...
            const playerList = game.players.map((p, i) => `${i + 1}. @${p.split("@")[0]}${i === game.turnIndex && game.started ? ' 🔄 (giliran)' : ''}`).join('\n');
    ...
    if (command === 'join') {
        if (game.started) {
            return await sock.sendMessage(remoteJid, { text: '⛔ Game sudah dimulai, tidak bisa bergabung lagi.' }, { quoted: message });
        }
        if (game.players.includes(sender)) {
            return await sock.sendMessage(remoteJid, { text: '⚠️ Kamu sudah bergabung.' }, { quoted: message });
        }
        if (game.players.length >= 10) {
            return await sock.sendMessage(remoteJid, { text: '🚫 Maksimal 10 pemain sudah tercapai.' }, { quoted: message });
        }

        game.players.push(sender);
        game.positions[sender] = 1;
        return await sendMessageWithMention(sock, remoteJid, `✅ @${sender.split("@")[0]} berhasil bergabung. Total pemain: ${game.players.length}`, message);
    ...
    if (command === 'start') {
        if (game.started) {
            return await sock.sendMessage(remoteJid, { text: '🟡 Game sudah dimulai.' }, { quoted: message });
        }
        if (game.players.length < 2) {
            return await sock.sendMessage(remoteJid, { text: '❌ Minimal 2 pemain untuk memulai permainan.' }, { quoted: message });
        }
        game.started = true;
        game.turnIndex = 0;
        return await sendMessageWithMention(sock, remoteJid, `🎲 Permainan dimulai!\nGiliran pertama: @${game.players[0].split("@")[0]} ketik ".snakes play" untuk lempar dadu.`, message);
    }

    // Play (lempar dadu)
    if (command === 'play') {
        if (!game.started) {
            return await sock.sendMessage(remoteJid, { text: '❌ Game belum dimulai. Ketik .snakes join dan .snakes start' }, { quoted: message });
        }

        if (game.players[game.turnIndex] !== sender) {
            return await sendMessageWithMention(sock, remoteJid, `🔄 Bukan giliranmu. Sekarang giliran: @${game.players[game.turnIndex].split("@")[0]}`, message);
        }
        ...
        if (game.positions[sender] === 100) {
            ...
            return await sendMessageWithMention(sock, remoteJid,`🏆 @${sender.split("@")[0]} menang! 🎉🎉\n\nAnda Dapat ${MONEY_MENANG} Money `, message);
        }
        ...
        try {
            ...
            const customizedMessage = `🎲 @${sender.split("@")[0]} melempar dadu: ${dice}\n📍 Posisi sekarang: ${game.positions[sender]} ${moveInfo}\n➡️ Giliran selanjutnya: @${game.players[game.turnIndex].split("@")[0]}`

            const result = await sendImagesWithMention(sock, remoteJid, buffer, customizedMessage, message);
