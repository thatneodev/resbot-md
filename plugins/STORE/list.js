...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        if (!firstElement) {
            ...
            if (first_checksetlist) { // jika ada setingan set list
                ...
                for (let line of lines) {
                    if (line.includes('@x')) {
                        ...
                        let listItems = keywordList2.map(item => `${template} ${item}`).join('\n'); // Buat daftar
        ...
        .replace(/@time/g, data.time);
                ...
                return await sendMessageWithMention(sock, remoteJid, message2, message);
            }
